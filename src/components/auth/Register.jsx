...
const Register = () => {
    const {google, apiLink, navigate } = useContext(Context)
    const form = useRef();
    ...
    const register = async (e) => {
      ...
      const confirmPassword = new FormData(form.current).get("password-confirm").trim();
      const name = {
     ...
     <div className="content">     
        <form className="auth-form" onSubmit={register} ref={form}>
          ...
          <fieldset>
            ...
            <input required name="name-first" id="name-first" type="text" />
          </fieldset>
          <fieldset>
            <label htmlFor="name-last">Last name</label>
            <input required name="name-last" id="name-last" type="text" />
          </fieldset>
          <fieldset>
        ...
        {errorMsg.length > 0 ? <div className="msg-error"><h3>{errorMsg}</h3></div> : null}
          <div className="buttons">
           
            <button>Sign up</button>
           
            {/* <button disabled className="btn-disabled">Sign up</button> */}

            
            <button onClick={google} className="google-btn">
